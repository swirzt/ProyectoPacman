...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        algo, visit = state
        e0,e1,e2,e3 = visit
        return e0 and e1 and e2 and e3
    ...
    def getSuccessors(self, state):
        ...
        Returns successor states, the actions they require, and a cost of 1.
         ...
         cost of expanding to that successor
        """
        actual, esquinas = (x,y), (v0, v1, v2, v3) = state
        e0, e1, e2, e3 = self.corners
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if ((nextx,nexty) == e0):
                    esquinasVisitadas = (True,v1,v2,v3)
                elif ((nextx,nexty) == e1):
                    esquinasVisitadas = (v0,True,v2, v3)
                elif ((nextx,nexty) == e2):
                    esquinasVisitadas = (v0,v1,True, v3)
                elif ((nextx,nexty) == e3):
                    esquinasVisitadas = (v0,v1,v2, True)
                else:
                    esquinasVisitadas = esquinas
                successors.append((((nextx,nexty), esquinasVisitadas), action, 1))
        self._expanded += 1
        return successors
...
def cornersHeuristic(state, problem):
    ...
    #         costos[i] = util.manhattanDistance(corners[i], actual)
    # minCosto = min(costos)
    ...
    # for i in range(4):   
    #     if not esquinasVisitadas[i]:
    #         esquinaX,esquinaY = corners[i]
    #         paredesActual = 0
    #         #Nunca ocurre que los 2 son iguales, solo uno o ninguno
    #         #Cuando vamos a la posicion la colocamos en visitados en otra funcion
    #         if esquinaX == x:
    #             for p in range(esquinaY,y,signo(y-esquinaY)):
    #                 if walls[x][p]:
    #                     paredesActual+=1
    #         elif esquinaY == y:
    #             for p in range(esquinaX,x,signo(x-esquinaX)):
    #                 if walls[p][y]:
    #                     paredesActual+=1
    #         else:    
    #             for p in range(esquinaX,x,signo(x-esquinaX)):
    #                 for j in range(esquinaY,y,signo(y-esquinaY)):
    #                     if walls[p][j]:
    #                         paredesActual+=1
